...
export const downloadImageAsPng = async (url: string, fullPath: boolean = false) => {
    ...
    //el buffer podemos usarlo para guardar el archivo, una vez hecho esto ya descargamos la imagen
    //fs.writeFileSync(`${folderPath}/${imageNamePng}`, buffer);

    const completePath = path.join(folderPath, imageNamePng);
    await sharp(buffer)
        .png()
        .ensureAlpha() //que tenga cierta configuración de colores para mandarlo a openai
        .toFile(completePath);
